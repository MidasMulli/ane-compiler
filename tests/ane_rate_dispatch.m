...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        if (argc < 2) { fprintf(stderr, "Usage: %s <model.mlmodelc> [ch] [iters] [rate_hz]\n", argv[0]); return 1; }
        loadFW();

        int ch = argc > 2 ? atoi(argv[2]) : 768;
        int iters = argc > 3 ? atoi(argv[3]) : 2000;
        // ⛔ RATE THROTTLE. The intercept-vs-slope question needs the SAME peripheral at DIFFERENT
        // dispatch rates: loss = INTERCEPT(substrate) + SLOPE * bytes_per_second, and two points at
        // one rate cannot separate them. Extrapolating a rate sweep to rate->0 isolates the
        // per-dispatch XPC cost, which is the first of the two candidates Paper 3 left open for its
        // -20.1%. usleep, never a spin: a busy-wait would contend through the CPU and be
        // indistinguishable from the effect under test.
        double rate = argc > 4 ? atof(argv[4]) : 0.0;
        useconds_t period_us = (rate > 0.0) ? (useconds_t)(1e6 / rate) : 0;
