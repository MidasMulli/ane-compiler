...
| **Llama 3.1-8B Q8** | 72 | **9.66 tok/s** | ANE | Single-stream at Q8, the deployment precision. Canonical, placement-verified (IOReport, n_a_reads=0, reproduced 9.53/9.66) on the 72-dispatch residual-capture *instrument* build, measured against a ~19.4 tok/s Q8 bandwidth floor (the gap is the dispatch tax of the instrumented topology, not a model limit). FP32 residual accumulation (FP16 fails past 16 layers at dim 4096), Llama 3 RoPE scaling. The earlier 7.9 figure is retired (silent-offload-confounded; see registry `llama-8b.throughput_ane`). A Q4 fused-multipass build reaches a higher ~13.4 tok/s at the lower precision (candidate; next row). |
| **Llama 3.1-8B fused attention** | 40 (MIL IR) | ~13.4 tok/s (Q4, candidate) | ANE (CPU_AND_NE) | Full attention incl. activation×activation matmul + softmax, SIP ON. The canonical claims here are correctness: fused-block dispatch count 40 and 5/5 prompt top-1 match vs PyTorch reference. The ~13.4 tok/s throughput is a **Q4 fused-multipass build** (registry `llama_3_1_8b_ane.fused_multipass_tok_s`, candidate, VERIFIED_ENERGY + ANEMLL, with real attention): a higher rate at the lower Q4 precision, explicitly NOT the Q8 deployment number (9.66, prior row). |
